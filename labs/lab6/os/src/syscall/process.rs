...
use crate::{
    config::MAX_SYSCALL_NUM, fs::{open_file, OpenFlags}, mm::{translated_byte_buffer, translated_refmut, translated_str, MapPermission, VirtAddr, VirtPageNum}, syscall::TASK_INFO_LIST, task::{
...
pub fn sys_spawn(_path: *const u8) -> isize {
    ...
    //这里和上一个lab不一样，我得到数据的方式是通过open_file，从文件系统里面读取
    if let Some(app_inode) = open_file(path.as_str(), OpenFlags::RDONLY) {
